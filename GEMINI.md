...
## Simplified Class Imports

- Always simplify class imports. Do not use Fully Qualified Class Names (FQCN) inline unless absolutely necessary for collision avoidance.
- Import classes at the top of the file using `use` statements.

## Secure Coding Practices

- Validate all inputs strictly using Form Requests or Filament validation.
- Output escaping: Ensure all Blade outputs are escaped ({{ }}) unless strictly intended to be raw ({!! !!}).
- Use Filament/Laravel Authorization: Always check permissions (e.g., `can()`) before performing actions.
- Avoid mass assignment vulnerabilities: Use `$fillable` or `unguard()` with extreme caution.
- Use Prepared Statements: Rely on Eloquent or Query Builder bindings; avoid raw SQL string concatenation.

## Filament 4 Artisan Commands

- Always use the dedicated Filament 4 Artisan commands for creating resources, pages, widgets, clusters, etc.
- Example: `php artisan make:filament-resource` instead of manually creating files.
- Consult `php artisan list` to verify available Filament commands.
