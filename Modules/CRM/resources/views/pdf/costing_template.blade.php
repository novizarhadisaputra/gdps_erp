...
<head>
    ...
    <title>Costing Template - {{ $record->name }}</title>
    <style>
        @page {
            margin: 0;
        }
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="doc-type" style="width: 50%; float: right;">
                ...
                <div class="doc-title">Costing Template</div>
        ...
        <div class="content">
            ...
            <table class="info-table">
                <tr>
                    ...
                    <td width="50%" style="padding-bottom: 12px; vertical-align: top;">
                        ...
                        <span class="info-value">{{ $record->costingTemplateItems->count() }} item</span>
                    </td>
                </tr>
                <tr>
                    <td width="50%" style="padding-bottom: 12px; vertical-align: top;">
                    ...
                    <td width="50%" style="padding-bottom: 12px; vertical-align: top;">
                        ...
                        <span class="info-value">{{ $record->updated_at->format('d M Y') }}</span>
                    </td>
                </tr>
            </table>
            ...
            <div class="section-title">{{ $record->description ? 'III.' : 'II.' }} Rincian Item & Biaya</div>

            <table class="items">
                <thead>
                    <tr>
                        <th width="4%">#</th>
                        <th width="28%">Nama Item / Deskripsi</th>
                        <th width="14%">Kategori</th>
                        <th class="center" width="5%">Qty</th>
                        <th class="right" width="13%">Base Price</th>
                        <th class="right" width="8%">Markup</th>
                        <th class="right" width="14%">Price (After Markup)</th>
                        <th class="right" width="14%">Monthly Cost</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($record->costingTemplateItems as $i => $item)
                        <tr>
                            <td>{{ $i + 1 }}</td>
                            <td>{{ $item->name }}</td>
                            <td><span
                            ...
                            <td class="right">{{ $item->markup_percent }}%</td>
                            <td class="right">Rp {{ number_format($item->unit_price_markup, 0, ',', '.') }}</td>
                            <td class="right">Rp {{ number_format($item->monthly_cost, 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="7" class="right"
                            style="font-size: 10px; letter-spacing: 0.5px; text-transform: uppercase;">Total Monthly
                            Cost Impact</td>
                        <td class="right" style="font-size: 13px;">Rp
                            {{ number_format($record->getTotalMonthlyCost(), 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
