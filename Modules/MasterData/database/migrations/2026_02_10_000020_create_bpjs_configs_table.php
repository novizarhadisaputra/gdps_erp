...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create(config('database.default') === 'sqlite' ? 'bpjs_configs' : 'master_data.bpjs_configs', function (Blueprint $table) {
            ...
            $table->foreignUuid('bpjs_basis_type_id')->nullable()->constrained(config('database.default') === 'sqlite' ? 'bpjs_basis_types' : 'master_data.bpjs_basis_types')->nullOnDelete();
            $table->decimal('employer_rate', 8, 4); // as percentage e.g. 0.037000
