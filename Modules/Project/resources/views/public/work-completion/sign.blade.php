    ...
    <div class="min-h-screen bg-gray-50 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl w-full space-y-8 bg-white p-8 rounded-2xl shadow-xl">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-lg font-extrabold text-gray-800">BAPP Signature Portal</p>
                    </div>
                </div>
            ...
            <div class="border-t border-b border-gray-100 py-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Work Completion Details</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        <p class="text-gray-500">Customer</p>
                        <p class="font-medium">{{ $report->customer?->name }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">Progress</p>
                        <p class="font-medium text-blue-600">{{ number_format($report->work_progress_percentage, 0) }}%</p>
                    </div>
                    <div>
                        <p class="text-gray-500">Period Start</p>
                        <p class="font-medium">{{ $report->service_period_start?->format('d M Y') }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">Period End</p>
                        <p class="font-medium">{{ $report->service_period_end?->format('d M Y') }}</p>
                    </div>
            ...
            <form action="{{ URL::temporarySignedRoute('work_completion_reports.public.submit', now()->addMinutes(60), ['report' => $report]) }}" method="POST" id="signature-form" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="email" name="sender_email" id="sender_email" required
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                               placeholder="e.g. youremail@company.com" value="{{ old('sender_email', $report->customer?->email) }}">
                    </div>
                    <div class="md:col-span-2">
                        <label for="signed_document" class="block text-sm font-medium text-gray-700">Signed BAPP File <span class="text-red-500">*</span></label>
    ...
    <div id="confirm-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-8">
            ...
            <p class="text-sm text-gray-600 mb-6">
                By clicking <strong>Confirm & Submit</strong>, you confirm that the work progress stated is accurate and agree to its terms. This action cannot be undone.
            </p>
