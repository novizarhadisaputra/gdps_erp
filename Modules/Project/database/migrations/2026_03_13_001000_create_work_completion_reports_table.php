...
return new class extends Migration
{
    ...
    {
        Schema::create(config('database.default') === 'sqlite' ? 'project_work_completion_reports' : 'project.work_completion_reports', function (Blueprint $blueprint) {
            $blueprint->uuid('id')->primary();
            $blueprint->foreignUuid('project_id')->constrained(config('database.default') === 'sqlite' ? 'project_projects' : 'project.projects')->onDelete('cascade');
            $blueprint->foreignUuid('product_cluster_id')->nullable()->constrained(config('database.default') === 'sqlite' ? 'master_data_product_clusters' : 'master_data.product_clusters')->onDelete('set null');
        ...
        Schema::create(config('database.default') === 'sqlite' ? 'project_work_completion_report_revisions' : 'project.work_completion_report_revisions', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('work_completion_report_id')->constrained(config('database.default') === 'sqlite' ? 'project_work_completion_reports' : 'project.work_completion_reports')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists(config('database.default') === 'sqlite' ? 'project_work_completion_report_revisions' : 'project.work_completion_report_revisions');
        Schema::dropIfExists(config('database.default') === 'sqlite' ? 'project_work_completion_reports' : 'project.work_completion_reports');
    }
};
